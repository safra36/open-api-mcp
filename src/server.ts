...
import { registerSio } from "./tools/sio.js";
import { registerBrowser } from "./tools/browser.js";
import { registerReport } from "./tools/report.js";
import { registerPrompts } from "./tools/prompts.js";

const INSTRUCTIONS = `This server tests live APIs across HTTP, WebSocket, Socket.IO, and a real browser, validating responses and frames against OpenAPI/AsyncAPI schemas.
...
4. Use auth_set / oauth_token for auth, ws_connect / ws_expect for raw WebSocket channels, sio_connect / sio_emit / sio_expect for Socket.IO (event-based) endpoints, and browser_* for UI flows (browser_capture_auth propagates a UI login to all planes).
5. Always finish by calling export_report({ format: "markdown" }) — this writes the full, detailed final report (contract/oracle, every assertion, each HTTP request+response with headers and bodies, WebSocket frames, browser network & console, session context) to test-report-ddmmyy.md in the working directory. Add junit/har/json/jest exports if the user wants them; read app://report at any time for the running tally.
